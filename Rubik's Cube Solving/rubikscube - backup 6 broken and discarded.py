...
def clear_redundant_moves(moves):
...
def dfs(cube_state, n: int, maxdepth: int, path=None, visited=None, depth=0):
    ...
    path = clear_redundant_moves(path)
    ...
    for i in range(6*n):
        ...
        if dfs(new_cube_state, n, maxdepth, path, visited, depth + 1): return (True, path)
        if path != []: path.pop()
    return False
...
my_cube_id = cube_to_str(my_cube, n)
print(my_cube_id)
print("Shuffle Pattern")
print(random_turns)
print("Solving...")
...
"""
003211322344553254015401
[(2, 3, -1), (2, 1, -1), (2, 2, -1), (2, 3, -1), (2, 1, 1)]
False
"""