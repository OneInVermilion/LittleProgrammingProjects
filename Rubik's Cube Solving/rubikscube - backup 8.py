...
def bfs(cube_state, n: int, maxdepth: int, path=None, visited=None, queue=None, depth=0):
    #currently outputs solutions, now we need to record and output path as well
    if path is None: path = []
    if visited is None: visited = set()
    if queue is None: queue = [0]
    counter = 0
    went_on_new_level = False
    while queue != [] and depth <= maxdepth:
        if queue == [0]: queue.pop()
        visited.add(cube_to_str(cube_state, n))
        if is_solved(cube_state):
        ...
        for i in range(6*n):
            potential_turn = num_to_turn(i)
            if (not went_on_new_level) and (path != []): path.pop() #removing last turn from path, turn from which we're switching to a neighbouring node
            path.append(potential_turn) #and adding the current node to path
            went_on_new_level = False
            new_cube_state = turn(cube_state, n, potential_turn[0], potential_turn[1], potential_turn[2])
            queue.append(new_cube_state)
            counter += 1
        cube_state = queue.pop(0)
        if counter == (6*n)**(depth+1): #if counter counts 6n (2nd depth layer queued) or 6n*6n (3rd layer queued) or...
            depth += 1
            went_on_new_level = True
...
if mode == 1: #solving
    ...
    my_cube = cube_from_str("442355230101101023442355", n)
    print_cube_values(my_cube, n)
    print(cube_to_str(my_cube, n))
    #print(random_turns)
    print(bfs(my_cube, n, s+1))
elif mode == 2: #testing
    my_cube_id = "442355230101101023442355"
    solution = [(2, 3, 1), (2, 3, 1), (2, 3, 1), (2, 3, 1)]
