...
def dfs(cube_state, n: int, maxdepth: int, path=None, visited=None, depth=0):
    ...
    for i in range(6*n):
        ...
        path.pop()
    visited.remove(cube_id) #EXPERIMENTAL
    return False
...
def bfs(cube_state, n: int, maxdepth: int):
    ...
    while queue:
        ...
        for i in range(6*n):
            ...
            if new_state_str not in visited: queue.append((new_state, new_path))
...
if mode == 1: #solving
    ...
    #my_cube = cube_from_str("241253301213020304555144", n) #Use this to generate determined cube, comment out for random cube
    print_cube_values(my_cube, n)
    print(cube_to_str(my_cube, n))
    print(random_turns)
    print("SOLVING NOW")
    print(dfs(my_cube, n, s))
elif mode == 2: #testing
    my_cube_id = "003003003211211211525525505414434434040242242151353353"
    solution = [(1, 3, -1), (1, 3, -1), (3, 3, -1), (1, 3, 1), (1, 1, 1), (2, 1, 1)]
